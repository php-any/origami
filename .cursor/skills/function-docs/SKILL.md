...
- **参数节点与绑定（配合 `param-types-docs` Skill）**
  - `Parameter`：
  ...
  - 参数的**语法解析**（联合类型、`?nullable`、`name: type`、引用、可变参数）由 `parser/parameter_parser.go` 完成，最终都落到 `data.Types` 上，具体规则见 `param-types-docs` Skill。
...
- **内置函数与方法（`std/php` 仅作参考示例）**
  ...
  - 文档中若需要「对照 PHP 语义给具体例子」，可以选取：
    - `std/php/empty.go` / `isset.go`：展示如何处理 AST 形式的参数、如何抑制错误或对 `null` 特判
    - `std/php/reflection/*`：展示内置方法（如 `ReflectionClass::newInstance`）如何通过 `GetParams` / `GetReturnType` 描述签名，并在 `Call` 里操作类、上下文和构造函数。
...
2. **再讲执行模型**
   - 调用时如何创建上下文、绑定参数（结合参数节点与 `data.Types`）
   - 如何处理返回值（包括返回值类型校验）
   - 如何处理 `yield` 生成器、异常与错误堆栈
3. **最后用 `std/php` 做例子**
   - 选一两个函数（如 `empty`、`isset`、`ReflectionClass::newInstance`）简单引用代码片段，说明语义是如何在运行时落地的。

## 文档编写建议

在项目的 `docs/` 中补充函数相关文档时，推荐拆成三个章节，并结合现有 Skill：

1. **运行容器与解释器背景**（引用 `interpreter-docs`）
...
3. **参数与类型系统**（引用 `param-types-docs`）
   - 参数语法（联合/可空/引用/可变参数）
   - `data.Types` 的核心概念与运行时检查

写文档时，把 `std/php` 当作「现成的、可信的示例代码库」，用于举例或验证理解；真正的规范应当来自 AST、运行容器和类型系统三者如何协同工作这一层。
