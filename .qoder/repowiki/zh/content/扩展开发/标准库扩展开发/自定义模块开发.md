...
```mermaid
graph TB
A["入口程序<br/>zy.go"] --> B["标准库加载器<br/>std/load.go"]
...
- 运行时与入口
  ...
  - 关键路径：[入口主流程:34-67](file://zy.go#L34-L67)、[标准库加载:14-38](file://std/load.go#L14-L38)。
- 模块加载器
...
participant Main as "入口程序(zy.go)"
...
- 文件不存在或路径错误
  ...
  - 参考：[路径检查与帮助输出:47-66](file://zy.go#L47-L66)。
- 模块未注册
