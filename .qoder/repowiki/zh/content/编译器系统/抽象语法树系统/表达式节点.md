...
- [node/binary_spaceship.go](file://node/binary_spaceship.go)
...
- [data/value_compare.go](file://data/value_compare.go)
</cite>

## 更新摘要
**所做更改**
- 新增三元比较运算符（Spaceship Operator）支持章节
- 更新二元运算符节点分析，包含新的 BinarySpaceship 实现
- 添加 Spaceship Operator 的语法解析和类型系统支持
- 更新依赖分析，包含新的比较运算符支持
...
**更新** 新增三元比较运算符（Spaceship Operator）支持，提供完整的二元比较实现和类型系统集成。
...
- [node/binary.go:1-98](file://node/binary.go#L1-L98)
- [node/binary_spaceship.go:1-64](file://node/binary_spaceship.go#L1-L64)
...
- 运算符节点：一元、二元、三元、空合并、数组构造、**三元比较运算符**
- 函数调用节点：即时调用与延迟解析调用

**更新** 新增三元比较运算符（Spaceship Operator）支持，提供完整的比较运算实现。
...
- **新增** 三元比较运算符：Spaceship Operator（<=>）支持数值和字符串比较，返回 -1、0 或 1
...
Dispatch --> |Spaceship| Spaceship["调用 BinarySpaceship.GetValue"]
Eval --> End(["返回二元节点"])
Rewrite --> End
Spaceship --> End
```
...
**图表来源**
- [node/binary.go:13-97](file://node/binary.go#L13-L97)
- [node/binary_spaceship.go:25-63](file://node/binary_spaceship.go#L25-L63)

**章节来源**
- [node/binary.go:1-98](file://node/binary.go#L1-L98)
- [node/binary_spaceship.go:1-64](file://node/binary_spaceship.go#L1-L64)

### 三元比较运算符（Spaceship Operator）
- **新增功能**：实现 PHP 7+ 的三元比较运算符（<=>），提供统一的比较接口
- 语法支持：在 token/type_config.go 中定义 SPACESHIP 操作符类型
- 类型系统：支持整数、浮点数、字符串、布尔值的比较
- 返回值：-1（左<右）、0（左==右）、1（左>右）
- 性能优化：针对整数类型进行特殊优化，直接进行数值比较

```mermaid
flowchart TD
S(["BinarySpaceship.GetValue"]) --> Left["求值左操作数"]
Left --> Right["求值右操作数"]
Right --> TypeCheck{"类型检查"}
TypeCheck --> |整数比较| IntCompare["直接数值比较"]
TypeCheck --> |可比较类型| GenericCompare["调用 data.Compare"]
TypeCheck --> |不可比较| DefaultReturn["返回 0"]
IntCompare --> Result["返回 -1/0/1"]
GenericCompare --> Result
...
**图表来源**
- [node/binary_spaceship.go:25-63](file://node/binary_spaceship.go#L25-L63)
- [data/value_compare.go:5-75](file://data/value_compare.go#L5-L75)

**章节来源**
- [node/binary_spaceship.go:1-64](file://node/binary_spaceship.go#L1-L64)
- [token/type_config.go:140](file://token/type_config.go#L140)
- [data/value_compare.go:1-76](file://data/value_compare.go#L1-L76)
...
- **新增** 类型系统依赖：Spaceship Operator 依赖 data/value_compare.go 中的 Compare 函数

```mermaid
graph LR
D["data 包<br/>值/上下文/控制/类型"] --> N["node.Node"]
N --> E["各表达式节点"]
N --> BS["BinarySpaceship"]
E --> D
BS --> DC["data.Compare<br/>(新增)"]
```
...
- [node/binary_spaceship.go:3-5](file://node/binary_spaceship.go#L3-L5)
- [data/value_compare.go:5-75](file://data/value_compare.go#L5-L75)
...
- **新增** 三元比较运算符优化
  - 整数类型直接比较：避免类型转换开销，提高比较效率
  - 通用比较：使用 data.Compare 函数，统一比较逻辑
- 类型检查与转换
...
- **新增** 三元比较运算符问题
  - 现象：比较结果不符合预期
  - 排查：确认操作数类型是否支持比较，检查 data.Compare 函数行为
  - 参考路径：[node/binary_spaceship.go:36-62](file://node/binary_spaceship.go#L36-L62), [data/value_compare.go:5-75](file://data/value_compare.go#L5-L75)
...
- [node/binary_spaceship.go:36-62](file://node/binary_spaceship.go#L36-L62)
- [data/value_compare.go:5-75](file://data/value_compare.go#L5-L75)

## 结论
表达式节点系统以统一的 GetValue 接口为核心，通过工厂与组合模式将各类运算与数据封装为可求值单元。其设计兼顾了 PHP 语义的兼容性与运行时性能，同时提供了清晰的错误与控制流处理路径。

**更新** 新增的三元比较运算符（Spaceship Operator）进一步增强了表达式节点系统的功能完整性，提供了标准的比较接口，支持多种数据类型的统一比较操作。该实现通过类型检查和优化策略，在保证正确性的同时提升了性能表现。

在实际使用中，建议：
- 明确操作数的类型与来源，优先使用变量引用与字面量
- 合理利用短路与空合并，减少无效求值
- 在热点路径中避免过度展开与复杂嵌套
- **新增** 利用 Spaceship Operator 进行高效的数值和字符串比较
- 通过来源信息与错误控制快速定位问题