...
- match
  - 表达式分支风格，支持多条件组合（逗号分隔）与 default。
  - 支持表达式体或语句体两种形式。
  - 未命中任何分支时返回空值。
- 测试验证
...
章节来源
- [控制结构文档:428-489](file://docs/control-structures.md#L428-L489)
...
章节来源
- [控制结构文档:491-542](file://docs/control-structures.md#L491-L542)

## 结论
Origami 的控制结构在语法层面贴近 PHP 习惯，在运行时通过节点与控制对象实现了清晰的执行模型。遵循本文的语法规范、最佳实践与故障排查建议，可在保证正确性的前提下提升代码性能与可维护性。建议在团队内统一条件书写风格、循环遍历方式与错误处理策略，以减少潜在陷阱。
