...
## 项目结构
仓库采用模块化组织，核心目录与职责如下：
- 根目录入口与构建：zy.go（CLI 入口）、go.mod（依赖与工具链）
...
```mermaid
graph TB
CLI["zy.go<br/>CLI入口"] --> Parser["parser/*<br/>语法解析"]
...
**图表来源**
- [zy.go:47-67](file://zy.go#L47-L67)
...
```mermaid
graph LR
origami_go["zy.go"] --> parser["parser.*"]
