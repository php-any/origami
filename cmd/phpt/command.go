...
示例:
  ...
  zy phpt --skip-deprecated=false   # 包含弃用相关用例`,
	...
	cmd.Flags().BoolVarP(&phptVerboseOutput, "verbose", "v", false, "输出 PASS/SKIP 明细")
	cmd.Flags().BoolVar(&phptShowSummary, "summary", false, "输出最终统计汇总")
	cmd.Flags().BoolVar(&phptFailFastExpect, "fail-fast-expect", true, "遇到 EXPECT/EXPECTF/EXPECTREGEX 不匹配时立即退出")
	cmd.Flags().BoolVar(&phptSkipDeprecated, "skip-deprecated", true, "跳过弃用（Deprecated）相关用例")
	return cmd
}
