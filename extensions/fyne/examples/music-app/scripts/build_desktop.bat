...
echo ============================================
echo Origami Music - Desktop Build
echo ============================================
echo.
...
set APP_DIR=extensions\fyne\examples\music-app
set BUILD_DIR=%APP_DIR%\build\desktop
set GEN_DIR=%APP_DIR%\build\gen
set OUT=%BUILD_DIR%\music_app.exe

REM Step 1: Generate app icon + Windows resource
...
REM Step 2: Compile PHP to Go (auto-discovers .zy/ templates)
echo [3/4] Compiling PHP to Go...
if exist "%GEN_DIR%" rmdir /S /Q "%GEN_DIR%"
go run zy.go compile %APP_DIR% --output="%GEN_DIR%" --pkg=musicapp --entry="%APP_DIR%\app.php"
if %ERRORLEVEL% NEQ 0 (
    echo === COMPILE FAILED ===
    pause
    exit /b 1
