...
func Generate(outputDir string) error {
	...
	if err := writeNamespaceFunctionIndexes(outputDir, modules); err != nil {
		return err
	}
...
// writeNamespaceFunctionIndexes 为每个含命名空间函数的目录生成 functions.php 索引（require 各函数 stub）。
func writeNamespaceFunctionIndexes(outputDir string, modules []PseudoCode) error {
	byNamespace := make(map[string][]string)
	seen := make(map[string]map[string]bool)

	for _, module := range modules {
		if module.Namespace == "" || len(module.Functions) == 0 {
			continue
		}
		if seen[module.Namespace] == nil {
			seen[module.Namespace] = make(map[string]bool)
		}
		for _, fn := range module.Functions {
			fileBase := strings.ToLower(fn.Name)
			if seen[module.Namespace][fileBase] {
				continue
			}
			seen[module.Namespace][fileBase] = true
			byNamespace[module.Namespace] = append(byNamespace[module.Namespace], fileBase)
		}
	}
	...
	for _, ns := range namespaces {
		files := byNamespace[ns]
		sort.Strings(files)

		dirPath := strings.ReplaceAll(ns, "\\", "/")
		fullDirPath := fmt.Sprintf("%s/%s", outputDir, dirPath)
		if err := os.MkdirAll(fullDirPath, 0o755); err != nil {
			return fmt.Errorf("创建目录 %s 失败: %w", fullDirPath, err)
		}

		indexPath := fmt.Sprintf("%s/functions%s", fullDirPath, pseudoCodeExt)
		if err := os.WriteFile(indexPath, []byte(generateFunctionIndexPHP(ns, files)), 0o644); err != nil {
			return fmt.Errorf("写入 %s 失败: %w", indexPath, err)
		}
		removeLegacyZyStub(indexPath)
	}
	return nil
}

func generateFunctionIndexPHP(namespace string, files []string) string {
	var buf strings.Builder
	buf.WriteString("<?php\n\nnamespace " + namespace + ";\n\n")
	buf.WriteString("// Auto-generated by gen-std. Function stubs:\n")
	for _, f := range files {
		buf.WriteString("//   - " + f + ".php\n")
		buf.WriteString("require_once __DIR__ . '/" + f + ".php';\n")
	}
	buf.WriteString("\n")
	return buf.String()
}
...
func buildModules(ctx data.Context, functions []data.FuncStmt, classes []data.ClassStmt) []PseudoCode {
	...
	if len(functions) > 0 {
		...
		for _, fn := range functions {
			...
			sig := analyzeFunction(fn, namespace)
			sig.Name = shortName

			if namespace == "" {
				globalFuncs.Functions = append(globalFuncs.Functions, sig)
				continue
			}

			modules = append(modules, PseudoCode{
				ModuleName:   strings.ToLower(shortName),
				Description:  fmt.Sprintf("%s 函数", fullName),
				Namespace:    namespace,
				IsAnnotation: isAnnotationNamespace(namespace),
				Functions:    []FunctionSignature{sig},
			})
		}

		if len(globalFuncs.Functions) > 0 {
			modules = append(modules, *globalFuncs)
		}
	}
...
func analyzeFunction(fn data.FuncStmt, namespace string) FunctionSignature {
	sig := FunctionSignature{
		Name: fn.GetName(),
	}

	sig.Params = analyzeMethodParams(fn, false, namespace)

	if returnTypeInterface, ok := fn.(data.GetReturnType); ok {
		rawReturnType := returnTypeInterface.GetReturnType()
		sig.ReturnType = formatPHPReturnType(rawReturnType, namespace, "")
		sig.ReturnDoc = formatPHPReturnDoc(rawReturnType, sig.ReturnType, namespace, "")
