...
func (s *illuminateRequestState) HTTPRequest() *nethttp.Request {
	if s == nil {
		return nil
	}
	return s.source
...
func initializeIlluminateRequest(cv *data.ClassValue, source *nethttp.Request) {
	if cv == nil {
		return
	}
	...
	if _, ctl := cv.GetProperty("query"); ctl != nil {
		_ = cv.SetProperty("query", httpfoundation.NewInputBagValue(cv, map[string]data.Value{}))
	}
	if source != nil {
		populateRequestFromHTTP(cv, source)
	}
	for name, maker := range map[string]func() data.Value{
		"query":      func() data.Value { return httpfoundation.NewInputBagValue(cv, nil) },
		"request":    func() data.Value { return httpfoundation.NewInputBagValue(cv, nil) },
		"attributes": func() data.Value { return httpfoundation.NewParameterBagValue(cv, nil) },
		"cookies":    func() data.Value { return httpfoundation.NewInputBagValue(cv, nil) },
		"files":      func() data.Value { return httpfoundation.NewFileBagValue(cv, nil) },
		"server":     func() data.Value { return httpfoundation.NewServerBagValue(cv, nil) },
		"headers":    func() data.Value { return httpfoundation.NewHeaderBagValue(cv, nil) },
	} {
...
func requestHTTPSource(cv *data.ClassValue) (*nethttp.Request, bool) {
	if cv == nil {
		return nil, false
	}
	...
	if sourceClass, ok := cv.Class.(data.GetSource); ok {
		if source, ok := sourceClass.GetSource().(*nethttp.Request); ok {
			return source, source != nil
		}
	}
	if r := httpfoundation.HTTPRequestFromValue(cv); r != nil {
		return r, true
	}
	return nil, false
}
...
func populateRequestFromHTTP(cv *data.ClassValue, r *nethttp.Request) {
	...
	_ = cv.SetProperty("query", httpfoundation.NewInputBagValue(cv, query))
	_ = cv.SetProperty("request", httpfoundation.NewInputBagValue(cv, form))
	_ = cv.SetProperty("attributes", httpfoundation.NewParameterBagValue(cv, nil))
	_ = cv.SetProperty("cookies", httpfoundation.NewInputBagValue(cv, cookies))
	_ = cv.SetProperty("files", httpfoundation.NewFileBagValue(cv, nil))
	_ = cv.SetProperty("server", httpfoundation.NewServerBagValue(cv, server))
	_ = cv.SetProperty("headers", httpfoundation.NewHeaderBagValue(cv, httpfoundation.HeadersFromRequest(r)))
}
...
func requestJSON(cv *data.ClassValue, key *string, fallback data.Value) (data.GetValue, data.Control) {
	...
	if state.json == nil {
		...
		state.json = httpfoundation.NewInputBagValue(cv, values)
	}
	if key == nil {
...
func invokeCallable(callable data.Value, args []data.Value, ctx data.Context) (data.GetValue, data.Control) {
	...
	case *data.BoundFuncValue:
		...
		return fn.Call(callCtx)
	default:
		if cv, ok := callable.(*data.ClassValue); ok && cv != nil {
			return invokeObject(cv, "__invoke", args, ctx)
		}
		return data.NewNullValue(), nil
	}
}
...
func setRequestBag(cv *data.ClassValue, name string, values map[string]data.Value) {
	...
	case "query", "request", "cookies":
		_ = cv.SetProperty(name, httpfoundation.NewInputBagValue(cv, values))
	case "attributes":
		_ = cv.SetProperty(name, httpfoundation.NewParameterBagValue(cv, values))
	case "files":
		_ = cv.SetProperty(name, httpfoundation.NewFileBagValue(cv, values))
	case "server":
		_ = cv.SetProperty(name, httpfoundation.NewServerBagValue(cv, values))
	}
}
...
func setInputSource(cv *data.ClassValue, values map[string]data.Value) {
	if isJSONRequest(cv) {
		bag := httpfoundation.NewInputBagValue(cv, values)
		requestState(cv).json = bag
		return
	}
	method := strings.ToUpper(requestMethod(cv))
...
func requestContent(cv *data.ClassValue) string {
	...
	// Symfony Request::create ç¼å­ body å¨ source.content
	if content := httpfoundation.RequestCachedContent(cv); content != "" {
		return content
	}
	// Illuminate Requestï¼ä¸ getContent() ä¸è´ï¼ä» nethttp.Request Body è¯»åï¼å¯éå¤è¯»ï¼
