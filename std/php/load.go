...
func Load(vm data.VM) {
	...
	for _, fun := range []data.FuncStmt{
		...
		core.NewComposerAutoloadFunction(),
		...
		core.NewOutputAddRewriteVarFunction(),
		core.NewObListHandlersFunction(),
		core.NewFlushFunction(),
		core.NewPrintRFunction(),
		...
		NewCallUserFuncArrayFunction(),
		NewMktimeFunction(),
		NewGetDefinedFunctionsFunction(),
		NewStreamWrapperRegisterFunction(),
		NewIniRestoreFunction(),
		NewIniAlterFunction(),
		NewDecHexFunction(),
		NewGetDeclaredClassesFunction(),
	} {
		vm.AddFunc(fun)
	}
