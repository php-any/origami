...
func NewFileBagValue(ctx data.Context, params map[string]data.Value) *data.ClassValue {
	...
	if params != nil {
		_ = fileBagReplace(ctx, inst, params)
	}
	return inst
}
...
func valueAsInt(v data.Value, def int) int {
	if v == nil {
		return def
	}
...
func constructNamedClass(ctx data.Context, fqn string, args ...data.Value) (*data.ClassValue, data.Control) {
	if ctx == nil || ctx.GetVM() == nil {
		return nil, data.NewErrorThrow(nil, fmt.Errorf("Class \"%s\" not found", fqn))
	}
	vm := ctx.GetVM()
	stmt, ok := vm.GetClass(fqn)
	if !ok || stmt == nil {
		loaded, ctl := vm.GetOrLoadClass(fqn)
		if ctl != nil {
			return nil, ctl
		}
		stmt = loaded
	...
	obj, ctl := stmt.GetValue(ctx.CreateBaseContext())
	if ctl != nil {
		return nil, ctl
	}
