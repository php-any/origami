...
func fileBagConstruct(ctx data.Context) (data.GetValue, data.Control) {
	...
	return nil, fileBagReplace(ctx, bagClassValue(ctx), m)
}
...
func fileBagReplace(ctx data.Context, cv *data.ClassValue, files map[string]data.Value) data.Control {
	...
	for k, v := range files {
		converted, ctl := convertFileInformation(ctx, v)
		if ctl != nil {
			return ctl
		}
		if converted != nil {
...
func fileBagSet(ctx data.Context) (data.GetValue, data.Control) {
	...
	converted, ctl := convertFileInformation(ctx, val)
	if ctl != nil {
		return nil, ctl
	}
	if converted == nil {
...
func convertFileInformation(ctx data.Context, file data.Value) (data.Value, data.Control) {
	...
	if equalStringSlices(keys, fileKeys) {
		errVal := 0
		if e, ok := m["error"]; ok {
			errVal = valueAsInt(e, 0)
		}
		if errVal == 4 { // UPLOAD_ERR_NO_FILE
			return nil, nil
		}
		tmp := ""
		if v, ok := m["tmp_name"]; ok && v != nil && !isNull(v) {
			tmp = v.AsString()
		}
		name := ""
		if v, ok := m["name"]; ok && v != nil && !isNull(v) {
			name = v.AsString()
		}
		if v, ok := m["full_path"]; ok && v != nil && !isNull(v) && v.AsString() != "" {
			name = v.AsString()
		}
		typ := ""
		if v, ok := m["type"]; ok && v != nil && !isNull(v) {
			typ = v.AsString()
		}
		// test=true，避免 Origami 无 is_uploaded_file 时 Laravel 上传失败
		return constructNamedClass(ctx, fqnUploadedFile,
			data.NewStringValue(tmp),
			data.NewStringValue(name),
			data.NewStringValue(typ),
			data.NewIntValue(errVal),
