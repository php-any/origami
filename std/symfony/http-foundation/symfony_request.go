...
var requestStaticState = struct {
	...
	trustedHeaderSet: -1,
	formats:          defaultRequestFormats(),
}

func defaultRequestFormats() map[string][]string {
...
func resetRequestFormatsState() {
	requestStaticState.Lock()
	requestStaticState.formats = defaultRequestFormats()
...
func requestSource(ctx data.Context) (*symfonyRequestSource, data.Control) {
	...
	if r := HTTPRequestFromValue(value); r != nil {
		return &symfonyRequestSource{
			request:       r,
			content:       bodyFromRequest(r),
			defaultLocale: "en",
			locale:        "en",
		}, nil
	}
