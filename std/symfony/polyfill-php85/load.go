...
// VendorRelFiles 是 Composer files autoload 会加载的相对路径（相对 vendor/symfony/polyfill-php85）。
...
// MarkLoaded 把 vendor/symfony/polyfill-php85/bootstrap.php 标进 phpFileCache。
func MarkLoaded(vm data.VM, vendorRoot string) {
	if vendorRoot == "" {
		return
	}
	base := filepath.Join(vendorRoot, "symfony", "polyfill-php85")
